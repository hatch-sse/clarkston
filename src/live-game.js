import { CLARKSTON_ROAD_ROWS } from './data/clarkstonRoads.js?v=20260529e';
...
const map = { roads: [], buildings: [], labels: [], source: '' };
let vehicles = [];
...
function roadWidth(highway) {
  if (highway === 'primary') return 52;
  if (highway === 'secondary') return 46;
  if (highway === 'tertiary') return 40;
  if (highway === 'service') return 24;
...
function parseRoads() {
  ...
  for (const row of CLARKSTON_ROAD_ROWS.split(';')) {
    ...
    if (name && main) {
      ...
      addLabel(name, mid.x, mid.y, false);
    }
  }
  map.source = `${map.roads.length} Clarkston roads loaded from bundled map data`;
}

function addLabel(label, x, y, important) {
  if (!label || map.labels.some(item => item.label === label && Math.hypot(item.x - x, item.y - y) < 120)) return;
  map.labels.push({ label, x, y, important });
}
...
function makeBuildings() {
  map.buildings = [];
  const taken = [];
  const roads = map.roads.filter(road => !road.foot && ['residential', 'unclassified', 'living_street', 'service'].includes(road.highway));
  for (let r = 0; r < roads.length && map.buildings.length < 1150; r += 1) {
    const road = roads[r];
    for (let i = 0; i < road.points.length - 1 && map.buildings.length < 1150; i += 1) {
      ...
      const step = road.width > 28 ? 76 : 62;
      for (let d = 32; d < length - 28 && map.buildings.length < 1150; d += step) {
        for (const side of [-1, 1]) {
          if (map.buildings.length >= 1150) break;
          const seed = r * 1000 + i * 37 + Math.round(d) + side * 19;
          const width = 18 + rand(seed) * 18;
          const depth = 16 + rand(seed + 3) * 16;
          const setback = road.width / 2 + depth / 2 + 16 + rand(seed + 9) * 8;
          ...
          if (isOnRoad(cx, cy, 22)) continue;
          if (taken.some(item => rectsOverlap(expandedBounds(bounds, 8), item))) continue;
          taken.push(bounds);
...
function placeVehicles() {
  ...
  for (const road of map.roads.filter(road => !road.foot && road.width >= 30)) {
    for (let i = 0; i < road.points.length - 1 && made < 42; i += 1) {
      const a = road.points[i];
      const b = road.points[i + 1];
      const length = Math.hypot(b.x - a.x, b.y - a.y);
      ...
      const angle = Math.atan2(uy, ux);
      for (let d = 75; d < length - 50 && made < 42; d += 260) {
...
function updateVehicle(vehicle, delta) {
  ...
  const accel = 430;
  if (throttle) vehicle.speed += accel * (boost ? 1.35 : 1) * delta;
  ...
  const turn = Math.min(1, Math.abs(vehicle.speed) / 155);
  vehicle.angle += steer * 2.75 * turn * Math.sign(vehicle.speed || 1) * delta;
  ...
  if (!isOnRoad(nextX, nextY, 28) || collidesWithBuilding(nextX, nextY)) {
    vehicle.speed *= -0.22;
    return;
  }
  vehicle.x = nextX;
...
function drawMap() {
  ...
  const paths = map.roads.filter(road => road.foot);
  for (const road of paths) if (visible(road.bounds)) drawPolyline(road.points, 'rgba(225,219,196,.22)', 3);
  ...
  for (const road of roads) if (road.width >= 38 && visible(road.bounds, 140)) drawPolyline(road.points, '#f3f0e8', 2, true, [20, 34]);
  drawLabels();
}
...
function drawMiniMap() {
  ...
  for (const road of map.roads.filter(road => !road.foot)) {
    miniCtx.lineWidth = Math.max(1, road.width * sx);
    miniCtx.beginPath();
    road.points.forEach((point, index) => index ? miniCtx.lineTo(point.x * sx, point.y * sy) : miniCtx.moveTo(point.x * sx, point.y * sy));
    miniCtx.stroke();
  }
  const target = mode === 'driving' && activeVehicle ? activeVehicle : player;
  miniCtx.fillStyle = '#45ff7b';
  miniCtx.beginPath();
...
function boot() {
  ...
  parseRoads();
  setLoading(45, 'Adding houses and parked cars...');
  makeBuildings();
  placeVehicles();
